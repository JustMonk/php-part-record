...
//добавляем список СЫРЬЯ ($materials), тут два селекта в WHERE т.к полуфабрикаты тоже используются в производстве полуфабрикатов
foreach ($mysqli->query('SELECT product_registry.registry_id, product_list.product_id, product_list.title, product_list.title, product_registry.count, units.unit, product_registry.create_date, product_registry.expire_date
FROM product_registry, product_list, units
WHERE product_registry.product_id = product_list.product_id AND product_list.unit_code = units.unit_id AND (product_list.product_type = (SELECT type_id FROM product_types WHERE TYPE = "Сырье") OR product_list.product_type = (SELECT type_id FROM product_types WHERE TYPE = "Полуфабрикат"))') as $row) {
...
foreach ($mysqli->query('SELECT product_registry.registry_id, product_list.product_id, product_list.title, product_list.title, product_registry.count, units.unit, product_registry.create_date, product_registry.expire_date
FROM product_registry, product_list, units
WHERE product_registry.product_id = product_list.product_id AND product_list.unit_code = units.unit_id AND (product_list.product_type = (SELECT type_id FROM product_types WHERE TYPE = "Полуфабрикат") OR product_list.product_type = (SELECT type_id FROM product_types WHERE TYPE = "Готовая продукция"))') as $row) {
